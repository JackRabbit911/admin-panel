...
const getSource7 = (pageNumber: number, lastPage: number) => (
    lastPage <= 7 ? short(pageNumber, lastPage) :
        [
...
const getSource5 = (pageNumber: number, lastPage: number) => (
    lastPage <= 5 ? short(pageNumber, lastPage) :
        [
            [pageNumber > 1, 1, false],
            [lastPage === pageNumber, pageNumber - 3, false],
            [(lastPage - pageNumber) < 2, pageNumber - 2, false],
            [pageNumber > 2, pageNumber - 1, false],
            [true, pageNumber, true],
            [(lastPage - pageNumber) >= 2, pageNumber + 1, false],
            [pageNumber < 3, pageNumber + 2, false],
            [pageNumber === 1, pageNumber + 3, false],
            [(lastPage - pageNumber) >= 1, lastPage, false],
        ]
    )

const preparePaginationData = (data: (string | number | boolean)[][]) => (
    data.filter((pageArray) => pageArray[0])
        .map((pageArray) => ({
            page: pageArray[1],
...
export const getPaginationData = ({ page, total, limit }: Props) => {
    ...
    const paginationData5 = preparePaginationData(getSource5(page, lastPage))
    const paginationData7 = preparePaginationData(getSource7(page, lastPage))

    return { paginationData5, paginationData7 }
}
